...
let mediumPassword = new RegExp(
  "((?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[^A-Za-z0-9])(?=.{6,}))|((?=.*[a-z])(?=.*[A-Z])(?=.*[^A-Za-z0-9])(?=.{8,}))|((?=.*[a-z])(?=.*[0-9])(?=.*[^A-Za-z0-9])(?=.{8,}))|((?=.*[a-z])(?=.*[0-9])(?=.*[A-Z])(?=.{8,}))"
);
...
function checkLeak(PasswordParameter) {
  if (leakPasswords.includes(PasswordParameter)) {
    strengthBadge.style.backgroundColor = "red";
    strengthBadge.textContent = "Password can be easily guessed";
    return;
  }
}
